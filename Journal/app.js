// Trading Journal Application - Enhanced with comprehensive psychology-focused fields
// IMPORTANT: no localStorage usage per strict instructions; data reset on refresh
...
class TradingJournalApp {
  constructor() {
    ...
    // Load sample trades for demo user1
    this.loadSampleData();
  ...
  loadSampleData() {
    const sampleTrades = [
      {
        id: 1,
        entryDate: '2025-07-25T09:30:00',
        exitDate: '2025-07-25T15:20:00',
        symbol: 'NIFTY',
        direction: 'Long',
        quantity: 100,
        entryPrice: 24850,
        ...
        // Psychology fields with sample data
        sleepQuality: 7,
        physicalCondition: 8,
        ...
        wouldTakeAgain: 'Definitely yes'
      },
      {
        id: 2,
        entryDate: '2025-07-26T10:15:00',
        exitDate: '2025-07-26T14:30:00',
        symbol: 'BANKNIFTY',
        direction: 'Short',
        quantity: 50,
        entryPrice: 51200,
        ...
        riskRewardRatio: 1.0,
        confidenceLevel: 4,
        notes: 'False breakdown, should have waited for confirmation',
        // Psychology fields
        sleepQuality: 4,
        physicalCondition: 5,
        ...
        positionComfort: 3,
        stressDuring: 9,
        primaryExitReason: 'Stop loss hit',
        exitEmotion: 'Frustrated',
        wouldTakeAgain: 'Probably no'
      },
      {
        id: 3,
        entryDate: '2025-07-27T09:45:00',
        exitDate: '2025-07-27T11:20:00',
        symbol: 'AAPL',
        direction: 'Long',
        quantity: 100,
        entryPrice: 230.5,
        ...
        stopLoss: 229,
        targetPrice: 235,
        strategy: 'Gap Up Play',
        exitReason: 'Manual Exit',
        grossPL: 225,
        netPL: 185,
        riskRewardRatio: 1.8,
        confidenceLevel: 7,
        notes: 'Good entry, partial target achieved',
        // Psychology fields
        sleepQuality: 8,
        physicalCondition: 7,
        ...
        positionComfort: 7,
        stressDuring: 4,
        primaryExitReason: 'Manual profit taking',
        exitEmotion: 'Satisfied',
        wouldTakeAgain: 'Probably yes'
      }
    ];

    const confidences = [
      { date: '2025-07-25', level: 8 },
      { date: '2025-07-26', level: 4 },
      { date: '2025-07-27', level: 7 }
    ];

    const user1 = this.users.find(u => u.id === 'user1');
    if (user1) {
      user1.trades = sampleTrades;
      user1.confidence = confidences;
    }
  }
  ...
  setupAuthListeners() {
    ...
    document.getElementById('loginFormElement').addEventListener('submit', e => {
      ...
      const iden = fd.get('username').trim();
      ...
      const user = this.users.find(u => (u.username === iden || u.email === iden) && u.password === pwd);
      if (!user) {
    ...
    document.getElementById('signupFormElement').addEventListener('submit', e => {
      ...
      if (this.users.some(u => u.username === username || u.email === email)) {
        this.showAuthError('signup-username-error', 'Username or email exists');
        return;
      }
      const newUser = { id: 'user_' + Date.now(), username, email, password: pwd, trades: [], confidence: [] };
    ...
    document.querySelectorAll('.demo-login').forEach(btn => {
      btn.addEventListener('click', () => {
        const user = this.users.find(u => u.username === btn.dataset.username);
  ...
  submitTrade() {
    ...
    this.currentUser.trades.push(trade);
